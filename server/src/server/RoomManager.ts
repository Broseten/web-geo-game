...
export class RoomManager {
    ...
    private roomTimeouts = new Map<string, NodeJS.Timeout>(); // A map to track the timeouts for empty rooms
    private playerRoomMap: Map<string, string>; // Map of socketId to roomId
    private roomDeletionTimeoutSeconds = 60;
    ...
    public joinRoom(clientSocket: Socket, roomID: string) {
        ...
        // Cancel the timeout if it exists because the player is reconnecting
        if (this.roomTimeouts.has(roomID)) {
            clearTimeout(this.roomTimeouts.get(roomID));
            this.roomTimeouts.delete(roomID);
            console.info(`Timeout for room ${roomID} canceled due to player reconnecting.`);
        }
    ...
    handleDisconnect(clientSocket: Socket) {
        ...
        if (roomId) {
            ...
            if (room) {
                ...
                // Clean up empty room
                if (room.isEmpty()) {
                    // Set a timeout to delete the room after 10 seconds and store it
                    const timeout = setTimeout(() => {
                        if (room.isEmpty()) {
                            this.rooms.delete(roomId);
                            console.info(`Room ${roomId} has been deleted.`);
                        }
                    }, this.roomDeletionTimeoutSeconds * 1000); // 10 seconds in milliseconds

                    this.roomTimeouts.set(roomId, timeout); // Store the timeout

                    console.info(`Room ${roomId} is empty and has been scheduled for deletion.`);
                }
            }
